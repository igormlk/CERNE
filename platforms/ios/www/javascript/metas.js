...
function carregarApresentacao(){
	db.transaction(function (tx) {
        tx.executeSql("SELECT * FROM CONFIG_KPI WHERE id_kpi = ? AND tipo_navegacao != 'G'", [cardNewId], function(tx, rs) {
...
function salvarLojas(id_kpi, lojas){
	db.transaction(function(tx) {
		...
		tx.executeSql(query, [tipoGrafico, lojas, id_kpi, seq_navegacao], function(tx, rs) {
			if(rs.item.lenght > 0){
				var query = "UPDATE LOJAS_KPI SET lojas = ? WHERE id_kpi = ?";
				tx.executeSql(query, [lojas, id_kpi], function(tx, rs) {
				...
				tx.executeSql(query, [id_kpi, lojas], function(tx, rs) {
